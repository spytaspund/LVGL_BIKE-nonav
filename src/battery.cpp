...
Battery::Battery(int addressPin, double convFactor, int reads)
{
    _reads = reads;
...
Battery::Battery(int addressPin, double convFactor)
{
    _reads = DEF_READS;
    _convFactor = convFactor;
    _addressPin = addressPin;
...
*/
int Battery::_getChargeLevel(double volts){
  int idx = 50;
  int prev = 0;
  int half = 0;
  if (volts >= 4.2){
